...
/* Görev 1: 
	...
	const istenenYil = fifaData.filter(yil =>{
		return yil["Stage"] == "Final" && yil["Year"] == 2014;})
	//console.log(istenenYil);

//(a) 2014 Dünya kupası Finali Evsahibi takım ismi (dizide "Home Team Name" anahtarı)

//console.log(istenenYil[0]['Home Team Name']);

//(b) 2014 Dünya kupası Finali Deplasman takım ismi  (dizide "Away Team Name" anahtarı)

//console.log(istenenYil[0]['Away Team Name']);

//(c) 2014 Dünya kupası finali Ev sahibi takım golleri (dizide "Home Team Goals" anahtarı)

//console.log(istenenYil[0]['Home Team Goals']);

//(d)2014 Dünya kupası finali Deplasman takım golleri  (dizide "Away Team Goals" anahtarı)

//console.log(istenenYil[0]['Away Team Goals']);

//(e) 2014 Dünya kupası finali kazananı*/

//console.log(istenenYil[0]['Win conditions'])
...
function Finaller(data) {
	const result = data.filter(final => {
...
function Yillar(data, callback) {
	const sonuc = callback(data).map(yil =>{
		return yil['Year']
	})
	return sonuc;
}
...
function Kazananlar(data, callback) {
	const kazananlar = callback(data).map(takim => {
		if(takim['Home Team Goals'] > takim['Away Team Goals']){
			return takim['Home Team Name']
		}
		else {
			return takim['Away Team Name']
		}	
	})
	return kazananlar;
}
